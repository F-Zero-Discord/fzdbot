...
async def get_connection_from_pool(pool: aiomysql.Pool):
    """Checks out a healthy connection from the pool; a connection whose ping
    fails is closed and handed back so the pool discards it.
    """
    conn = None
    try:
...
async def get_db_connection():
    ...
    conn = None
    try:
        conn = await get_connection_from_pool(pool)
...
async def execute_query(conn, query, params=None, fetch: str | None = "all", is_proc: bool = False):
...
async def create_event(db, event, duration: int = 2) -> None:
    ...
    """
    now = datetime.now(UTC)  # now.strftime('%Y-%m-%d %H:%M:%S')
...
async def get_event_schedule(db):
    ...
    events = await execute_query(db, sql_events, params=None, fetch="all", is_proc=False)
    # print(events)
    # print(type(events))
    return events
